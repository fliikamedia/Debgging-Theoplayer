...
export class ThumbnailView extends PureComponent {
  ...
  renderPlaceHolderThumbnail = index => {
    ...
    return (
      <View
        key={`th${index}`}
        style={[styles.thumbnail, {width: size, height: 1}, style]}
      />
    );
  };
  ...
  renderThumbnail = (thumbnail, index) => {
    ...
    const {
      size,
      carouselThumbnailScale,
      thumbnailStyleCurrent,
      thumbnailStyleCarousel,
    } = this.props;
    ...
    if (isTileMapThumbnail(thumbnail)) {
      const ratio =
        thumbnail.tileWidth == 0 ? 0 : (scale * size) / thumbnail.tileWidth;
      return (
        <View
          key={`th${index}`}
          style={[
            styles.thumbnail,
            {width: scale * renderWidth, height: scale * renderHeight},
            style,
          ]}>
          <Image
            resizeMode={'cover'}
  ...
  render() {
    ...
    for (let i = 0; i < carouselThumbCount; i++) {
      ...
      before.push(
        beforeIndex >= 0
          ? this.getThumbnailImageForCue(thumbnailTrack.cues[beforeIndex])
          : null,
      );
      const afterIndex = nowCueIndex + i + 1;
      after.push(
        afterIndex < thumbnailTrack.cues.length
          ? this.getThumbnailImageForCue(thumbnailTrack.cues[afterIndex])
          : null,
      );
    }
    const current = this.getThumbnailImageForCue(
      thumbnailTrack.cues[nowCueIndex],
    );
    ...
    return (
      <View style={{flexDirection: 'column'}}>
        ...
        <View
          style={[
            styles.container,
            {height: renderHeight, marginLeft: offset ?? 0},
            containerStyle,
          ]}>
