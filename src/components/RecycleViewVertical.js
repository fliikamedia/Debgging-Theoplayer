...
export default class RecycleViewVertical extends Component {
  constructor(props) {
    ...
    this.layoutProvider = new LayoutProvider(
      ...
      (type, dim) => {
        switch (type) {
          case "NORMAL":
            dim.width = SCREEN_WIDTH * 0.32;
            dim.height = SCREEN_WIDTH * 0.48;
            break;
          default:
            dim.width = 0;
            dim.height = 0;
    ...
    this.rowRenderer = (type, item) => {
      const { dvd_thumbnail_link, _id, film_type, title, season_number } =
        item.item;
  ...
  render() {
    ...
    if (this.props.movie.length > 3 && this.props.movie.length % 3 !== 0) {
      heights =
        SIZES.width * 0.45 * (this.props.movie.length / 3) + SIZES.width * 0.7;
    } else if (
      this.props.movie.length > 3 &&
      this.props.movie.length % 3 === 0
    ) {
